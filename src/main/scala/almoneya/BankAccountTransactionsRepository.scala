...
class BankAccountTransactionsRepository(executor: QueryExecutor) {
    ...
    def importBankTransactionsTransactions(tenantId: TenantId, bankAccounts: Set[BankAccount], transactions: Set[BankAccountTransaction]): Try[Set[BankAccountTransaction]] = {
        executor.findAll(Query("SELECT bank_account_hash FROM bank_accounts")) { rs =>
            ...
            val values: Seq[Seq[SqlValue]] = transactions.map(txn => Seq[SqlValue](tenantId, txn.bankAccount.accountNum, txn.postedOn, txn.description1, txn.description2, txn.checkNum, txn.amount)).toSeq
...
object BankAccountTransactionsRepository {
    val importBankTransactionSql = Query("INSERT INTO bank_account_transactions(tenant_id, bank_account_hash, posted_on, description1, description2, check_number, amount) VALUES",
        Seq(Column("bank_account_transaction_id"), Column("bank_account_hash"), Column("posted_on"), Column("description1"), Column("description2"), Column("check_number"), Column("amount"), Column("created_at"), Column("updated_at")))
}

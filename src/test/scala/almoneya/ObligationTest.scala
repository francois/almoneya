...
class ObligationTest extends FunSuite {
    ...
    private[this] def newWeeklyObligation(startOn: String, endOn: Option[String] = None) =
        Obligation(envelope = envelope, startOn = new LocalDate(startOn), endOn = endOn.map(new LocalDate(_)), amount = zero, every = Every(1), period = Weekly)

    private[this] val envelope = Envelope(name = EnvelopeName("envelope"))
